(function () {
    ...
    const DEFAULT_CONFIG = {
        ...
        acceptChance: 70, randomInviteChance: 7,
        background: { mode: 'gradient', colorA: '#e9e3dc', colorB: '#d6ddd4', image: '', imageOpacity: 0.28 },
        music: { rainUrl: '', fireUrl: '', customUrl: '', customName: '自定义歌曲' }
    };
    ...
    function injectUI() {
        ...
        root.innerHTML = `
            ...
            <section class="cp-screen cp-room" id="cp-room-screen">
                ...
                <section class="cp-full-page" id="cp-history-page">
                ...
                <section class="cp-full-page" id="cp-chat-page">
                    <header class="cp-full-head"><button class="cp-glass" data-cp-page-close><i class="fas fa-chevron-left"></i></button><b>陪伴中说说话</b><span></span></header>
                    <div class="cp-session-list" id="cp-chat-list"></div>
                    <div class="cp-session-compose"><input class="cp-input" id="cp-chat-input" maxlength="160" placeholder="说点什么…"><button class="cp-btn primary" id="cp-chat-send"><i class="fas fa-paper-plane"></i></button></div>
                </section>
                <section class="cp-full-page" id="cp-settings-page">
                    ...
                    <div class="cp-settings-body">
                        ...
                        <div class="cp-setting-block"><h3>音乐链接</h3><p>保留雨声、篝火和自定义歌曲按钮；本站不再内置任何音频，请在这里填写自己的链接并固定保存。</p>
                            <div class="cp-url-row"><span>雨声</span><input class="cp-input" id="cp-rain-url" placeholder="填写雨声音频链接"></div>
                            <div class="cp-url-row"><span>篝火</span><input class="cp-input" id="cp-fire-url" placeholder="填写篝火音频链接"></div>
                            <div class="cp-url-row"><span>歌曲名称</span><input class="cp-input" id="cp-custom-name" placeholder="自定义歌曲"></div>
                            <div class="cp-url-row"><span>歌曲链接</span><input class="cp-input" id="cp-custom-url" placeholder="填写音乐链接"></div>
                        </div>
                        <div class="cp-setting-block"><h3>邀约概率</h3><p>梦角会在主聊天正常回复结束后，偶尔随机发出邀约。</p>
                            <label class="cp-label">接受概率 <span id="cp-accept-value"></span></label><input class="cp-range" id="cp-accept-chance" type="range" min="0" max="100">
                            <label class="cp-label">随机发起概率 <span id="cp-random-value"></span></label><input class="cp-range" id="cp-random-chance" type="range" min="0" max="30">
                        </div>
                    </div>
                </section>
            </section>

            <div class="cp-overlay" id="cp-activity-overlay"><div class="cp-panel"><div class="cp-handle"></div><div class="cp-panel-head"><div><b>发起陪伴</b><small>选择自己的陪伴状态</small></div><button class="cp-close" data-cp-overlay-close>×</button></div><div class="cp-panel-pad"><div class="cp-grid" id="cp-state-grid"></div><button class="cp-test-link" id="cp-open-pre-settings"><i class="fas fa-cog"></i> 陪伴设置（状态图片、背景与音乐链接）</button><button class="cp-test-link" id="cp-preview-incoming">预览测试：让梦角发来一次邀约</button></div></div></div>
            ...
            <div class="cp-overlay" id="cp-music-overlay"><div class="cp-panel"><div class="cp-handle"></div><div class="cp-panel-head"><div><b>陪伴音乐</b><small id="cp-music-status">当前静音</small></div><button class="cp-close" data-cp-overlay-close>×</button></div><div class="cp-panel-pad"><div class="cp-music-grid"><button class="cp-music-btn" data-music="rain"><i class="fas fa-cloud-rain"></i>雨声</button><button class="cp-music-btn" data-music="fire"><i class="fas fa-fire"></i>篝火</button><button class="cp-music-btn" data-music="custom"><i class="fas fa-music"></i><span id="cp-custom-music-label">自定义歌曲</span></button></div><label class="cp-label">音量 <span id="cp-volume-value">45%</span></label><input class="cp-range" id="cp-volume" type="range" min="0" max="100" value="45"><button class="cp-test-link" id="cp-stop-music">停止播放</button></div></div></div>
    ...
    function bindEvents() {
        byId('companion-btn')?.addEventListener('click', openActivityPicker);
        ...
        byId('cp-duration-back').addEventListener('click', () => { closeOverlays(); if (durationPurpose === 'invite') openActivityPicker(); });
        byId('cp-duration-confirm').addEventListener('click', confirmDuration);
        byId('cp-preview-incoming').addEventListener('click', () => { closeOverlays(); createPartnerInvitation(true); });
        byId('cp-open-pre-settings').addEventListener('click', () => { closeOverlays(); openRootScreen('cp-room-screen'); applyBackground(); openSettings(); });
        ...
        byId('cp-music-open').addEventListener('click', () => openOverlay('cp-music-overlay'));
        ...
        ['rain', 'fire', 'custom'].forEach(key => byId(`cp-${key}-url`)?.addEventListener('change', saveMusicInputs));
        byId('cp-custom-name').addEventListener('change', saveMusicInputs);
        ...
        byId('cp-accept-chance').addEventListener('input', event => { config.acceptChance = Number(event.target.value); byId('cp-accept-value').textContent = `${event.target.value}%`; saveConfig(); });
        byId('cp-random-chance').addEventListener('input', event => { config.randomInviteChance = Number(event.target.value); byId('cp-random-value').textContent = `${event.target.value}%`; saveConfig(); });
    ...
    function openActivityPicker() {
        ...
        renderStateGrid(); byId('companion-root').classList.add('open'); openOverlay('cp-activity-overlay');
    }

    function renderStateGrid() {
        const grid = byId('cp-state-grid'); grid.innerHTML = '';
        states().forEach(item => {
            const button = document.createElement('button'); button.className = 'cp-choice'; button.dataset.state = item.id;
            button.innerHTML = `<i class="fas ${item.icon || 'fa-star'}"></i><b></b><small>选择后设置陪伴时间</small>`;
            button.querySelector('b').textContent = item.name;
            button.addEventListener('click', () => openDuration(item.id, 'invite'));
    ...
    function sendUserInvitation(stateId, minutes) {
        ...
        setTimeout(() => {
            ...
            const accepted = Math.random() * 100 < Number(config.acceptChance);
    ...
    function createPartnerInvitation(force) {
        ...
        if (!force && Math.random() * 100 >= Number(config.randomInviteChance)) return;
    ...
    function openSessionChat() { renderLogs(); byId('cp-chat-page').classList.add('open'); setTimeout(() => byId('cp-chat-input').focus(), 200); }
    function renderLogs() {
        if (!session) return; ['cp-history-list', 'cp-chat-list'].forEach(id => {
            const list = byId(id); list.innerHTML = '';
            session.logs.forEach(item => {
                const row = document.createElement('div');
                if (item.type === 'system') { row.className = 'cp-log-system'; row.textContent = item.text; }
                else { row.className = `cp-log-line ${item.type}`; const bubble = document.createElement('div'); bubble.className = 'cp-log-bubble'; bubble.textContent = item.text; row.appendChild(bubble); }
    ...
    function sendSessionChat() {
        ...
        input.value = ''; session.logs.push({ type: 'me', text }); renderLogs();
        ...
        for (let i = 0; i < count; i++) {
            ...
            setTimeout(() => {
                ...
                session.logs.push({ type: 'partner', text: reply }); renderLogs(); bridge().playMessageSound?.();
            }, wait);
        }
    }
    ...
    function renderSettings() {
        renderStateSelects(); renderImageMap(); const bg = config.background, music = config.music;
        ...
        byId('cp-rain-url').value = music.rainUrl; byId('cp-fire-url').value = music.fireUrl; byId('cp-custom-url').value = music.customUrl; byId('cp-custom-name').value = music.customName;
        byId('cp-accept-chance').value = config.acceptChance; byId('cp-accept-value').textContent = `${config.acceptChance}%`; byId('cp-random-chance').value = config.randomInviteChance; byId('cp-random-value').textContent = `${config.randomInviteChance}%`;
    }
    ...
    function saveMusicInputs() {
        config.music.rainUrl = byId('cp-rain-url').value.trim(); config.music.fireUrl = byId('cp-fire-url').value.trim(); config.music.customUrl = byId('cp-custom-url').value.trim(); config.music.customName = byId('cp-custom-name').value.trim() || '自定义歌曲'; byId('cp-custom-music-label').textContent = config.music.customName; saveConfig(); notify('音乐链接已保存');
    }

    function playMusic(type) {
        const key = `${type}Url`; const url = config.music[key]; if (!url) { notify(`请先在设置中填写${type === 'rain' ? '雨声' : type === 'fire' ? '篝火' : '歌曲'}链接`); return; }
        const audio = byId('cp-audio'); audio.src = url; audio.volume = Number(byId('cp-volume').value) / 100;
        audio.play().then(() => { currentMusic = type; all('[data-music]').forEach(button => button.classList.toggle('active', button.dataset.music === type)); byId('cp-music-status').textContent = `正在播放 · ${type === 'rain' ? '雨声' : type === 'fire' ? '篝火' : config.music.customName}`; }).catch(() => notify('音乐链接无法播放，请检查是否为可直接访问的音频地址', 'error'));
    }
    ...
    function requestContinuation(minutes) {
        ...
        setTimeout(() => {
            if (!session) return; const accepted = Math.random() * 100 < Number(config.acceptChance);
    ...
    function finishSession() { if (!session) return; clearInterval(countdownTimer); clearInterval(rarePartnerTimer); stopMusic(); session = null; closeRoot(); notify('本次陪伴已经结束'); }
    ...
    async function initialize() {
        ...
        config = mergeConfig(await readStored()); injectUI(); bindEvents(); renderStateGrid(); applyBackground(); byId('cp-custom-music-label').textContent = config.music.customName; initialized = true;
    }
